    ...
    <div class="taskBox">
      ...
      <div class="taskItem" v-if="taskListMsg.length > 0" v-for="(taskItem, index) in taskListMsg" v-bind:key="index" @click="toTaskDetailPage(taskItem.uid)">
        <div class="taskLf">
          ...
          <div class="taskPro"><span class="label">创</span>&nbsp;{{taskItem.createrName}}</div>
          <div class="taskMsg">
...
export default {
  ...
  methods: {
    ...
    getTaskCondition: function () {
      var that = this
      that.ajax('/app/getTaskCondition', {userId: that.userId}).then(res => {
        if (res.code === 200) {
